...
def triangleFill(point1, point2, point3, spacing):
    ...
    (x2, y2, z2) = point2
    (x3, y3, z3) = point3
    distance = math.fabs((y2 - y1) * x3 - (x2 - x1) * y3 - y2 * x1) / math.sqrt(
        (y2 - y1) ** 2 + (x2 - x1) ** 2
    ...
    scale_major = 1
    while (
        dist <= (distance - spacing)
    ):  # each step we move along vector 1-2 by an amount that scales down according to how far we've gone. Use scale transformation
        # first major movement along 1-2 vector
        relative_scale_major = scaleMajor(
            angle1, angle2, initDist, spacing
        )  # calculating absolute vs. relative scaling, I will want to switch this to relative for rhombus to work easily
        ...
        g.move(
            (x2 - x1) * scale_major * sign,
            (y2 - y1) * scale_major * sign,
            (z2 - z1) * scale_major * sign,
        )
        ...
        scale_major = (
            scale_major * relative_scale_major
        )  # should let it go down then up again if necessary
...
def rhombusFill(point1, point2, point3, point4, spacing):
    ...
    (x3, y3, z3) = point3
    (x4, y4, z4) = point4
    distance_3 = math.fabs((y2 - y1) * x3 - (x2 - x1) * y3 - y2 * x1) / math.sqrt(
        (y2 - y1) ** 2 + (x2 - x1) ** 2
    )  # currently only works in 2D!!!
    distance_4 = math.fabs((y2 - y1) * x4 - (x2 - x1) * y4 - y2 * x1) / math.sqrt(
        (y2 - y1) ** 2 + (x2 - x1) ** 2
    )  # currently only works in 2D!!!
    initDist = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2 + (z2 - z1) ** 2)
    vector1_2 = np.array(point2) - np.array(point1)
    ...
    scale_minor1 = scaleMinor(
        angle1, spacing, point1, point4
    )  # minor scale values are constant between points
    scale_minor2 = scaleMinor(angle2, spacing, point2, point3)
    ...
    if distance_4 <= distance_3:
        while (
            dist <= (distance_3 - spacing)
        ):  # each step we move along vector 1-2 by an amount that scales down according to how far we've gone. Use scale transformation
            # first major movement along 1-2 vector
            if dist <= distance_4 - spacing:
                relative_scale_major = scaleMajor(
                    angle1, angle2, initDist, spacing
                )  # relative scaling of next step vs. prior step
            else:
                relative_scale_major = scaleMajor(
                    (angle4 - (math.pi - angle1)), angle2, initDist, spacing
                )  # relative scaling of next step vs. prior step
            g.move(
                (x2 - x1) * scale_major * sign,
                (y2 - y1) * scale_major * sign,
            ...
            scale_major = (
                scale_major * relative_scale_major
            )  # applies new scaling on top of previous one
            initDist = initDist * relative_scale_major  # length of previous line

    else:
        while (
            dist <= (distance_4 - spacing)
        ):  # each step we move along vector 1-2 by an amount that scales down according to how far we've gone. Use scale transformation
            # first major movement along 1-2 vector
            if dist <= distance_3 - spacing:
                ...
                relative_scale_major = scaleMajor(
                    angle1, (angle3 - (math.pi - angle2)), initDist, spacing
                )  # relative scaling of next step vs. prior step
            g.move(
                (x2 - x1) * scale_major * sign,
                (y2 - y1) * scale_major * sign,
...
def triangleMultilayerMeander(
    rotation, com, speed, pres, point2, point3, z, spacing, layers
):
    g.push_matrix()
    g.rotate(rotation)
    g.feed(speed)
...
def rhombusMultilayerMeander(
    rotation, com, speed, pres, point2, point3, point4, z, spacing, layers
):
    g.push_matrix()
    g.rotate(rotation)
    g.feed(speed)
...
def moveRotationCircumferential(rotation, r, ew, layers):
    ...
    g.rotate(rotation)
    g.move(r, 0, 0)
    triangleMultilayerMeander(
        0, com_LCE, spd_LCE, LCEpres, (0, ew, 0), (-1 * r, 0, 0), z_LCE, spc_LCE, layers
    )
    triangleMultilayerMeander(
        0,
        com_LCE,
...
def moveRotationRadial(rotation, r, ew, layers):
    ...
    g.move(r, 0, 0)
    triangleMultilayerMeander(
        0, com_LCE, spd_LCE, LCEpres, (-1 * r, 0, 0), (0, ew, 0), z_LCE, spc_LCE, layers
    )
    triangleMultilayerMeander(
        0,
        com_LCE,
...
while j <= sections:
    ...
    moveRotationCircumferential(
        j * 2 * angle_step_rad, radius, end_width, layers_circumferential
    )
    j = j + 1
