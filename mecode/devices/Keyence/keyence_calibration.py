...
tests = [[0,1,2],
		 ...
		 [2,3,0],
		 [3,0,1]]
summed_intersection_point = np.array([0.0,0.0,0.0])
for test in tests:
	planes = []
	for index, side in enumerate(sides):
		...
		if index in test:
			...
			print("Side {} composed of {} segments".format(index,len(largestContour)))
	...
	print("The location of the pyramid using sides ({},{},{}) is:".format(*test))
	intersection_point = intersection_of_planes(planes)
	summed_intersection_point += np.array(intersection_point)
	print("x:{:.2f} y:{:.2f} z:{:.2f}".format(*intersection_point))
print("The average location of the pyramid is:")
print("x:{:.2f} y:{:.2f} z:{:.2f}".format(*summed_intersection_point/len(tests)))
